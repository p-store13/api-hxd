...
router.post('/signup', recaptcha.middleware.verify, captchaRegister, async(req, res) => {
    ...
    if (!email || !username || !password || !confirmpassword) {
        req.flash('error_messages','Semua Bidang Diperlukan !');
        res.redirect('/signup');
        ...
        user.findOne({ $or: [{ email: email }, { username: username }] }, function (err, data) {
            if (err) throw err;
            if (data) {
                req.flash('error_messages',"Pengguna Sudah Terdaftar, Silakan Masuk !");
                res.redirect('/signup');
            } else {
                bcryptjs.genSalt(12, (err, salt) => {
                    ...
                    bcryptjs.hash(password, salt, (err, hash) => {
                        ...
                        user({
                            ...
                            if (err) throw err;
                            req.flash('success_messages',"Akun Berhasil Dibuat Silakan Masuk");
                            res.redirect('/login');
                        });
                    })
                });
...
router.get('/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified ) {
        ...
        req.flash('success_messages', 'Tautan Email Berhasil Dikirim Dan Akan Kadaluarsa Dalam 30 menit.')
        res.redirect('/docs');
        }
...
router.get('/verifyemail', async (req, res) => {
    ...
    if (token) {
        ...
        if (check) {
            ...
            req.flash('error_messages', 'Tautan Kedaluwarsa Atau Error')
            res.redirect('/login');
        }
    }
    } else {
...
if (userData) {
if (Cooldown) {
    req.flash('error_messages','Mohon Jangan Spam Tunggu Sampai 30 menit.');
    res.redirect('/forgot-password')
            ...
            req.flash('success_messages','Cek email Anda untuk info lebih lanjut, tunggu 30 menit.');
            res.redirect('/forgot-password');    
            }
...
router.get('/reset-password', recaptcha.middleware.render, async (req, res) => {
    ...
    if (token) {
        ...
        if (check) {
            ...
            req.flash('error_messages','Token Rusak Atau Sudah Kadaluarsa.');
            res.redirect('/forgot-password');
        }
    } else {
        res.redirect('/login');
...
var checkpw = resetpw.validate(password)
    ...
    if (!password || !confirmpassword || confirmpassword != password) {
        ...
        if (salt) {
            ...
            req.flash('success_messages', 'Kata Sandi Telah Berubah')
            res.redirect('/login');
        } else {
        req.flash('error_messages',"Kesalahan Tidak Diketahui Silakan Coba Lagi");
        res.redirect(`/reset-password?token=${token}`);
        }
    }
});
